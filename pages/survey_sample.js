import SurveyQuestion from "../../next-tutorial/components/questions/SurveyQuestion";
import Styles from '../styles/survey.module.css'
const Survey = () => {
    const survey = [
        {
            "question": "How likely is it that you would recommend this company to a friend or colleague?",
            "question_type": "r11",
            "question_id": "q00",
            "question_category": "nps",
            "question_select": "single",
            "question_high_low":{
                "high":"Not at all Likely",
                "low": "Extremely Likely"
            },
            "question_responses": [
                {
                    "value": "0",
                    "text": "0"
                },
                {
                    "value": "1",
                    "text": "1"
                },
                {
                    "value": "2",
                    "text": "2"
                },
                {
                    "value": "3",
                    "text": "3"
                },
                {
                    "value": "4",
                    "text": "4"
                },
                {
                    "value": "5",
                    "text": "5"
                },
                {
                    "value": "6",
                    "text": "6"
                },
                {
                    "value": "7",
                    "text": "7"
                },
                {
                    "value": "8",
                    "text": "8"
                },
                {
                    "value": "9",
                    "text": "9"
                },
                {
                    "value": "10",
                    "text": "10"
                }
            ]
        },
        {
            "question": "Overall, how satisfied or dissatisfied are you with our company?",
            "question_type": "r5",
            "question_id": "q01",
            "question_category": "company",
            "question_select": "single",
            "question_responses": [
                {
                    "value": "0",
                    "text": "Very satisfied"
                },
                {
                    "value": "1",
                    "text": "Somewhat satisfied"
                },
                {
                    "value": "2",
                    "text": "Neither satisfied nor dissatisfied"
                },
                {
                    "value": "3",
                    "text": "Somewhat dissatisfied"
                },
                {
                    "value": "4",
                    "text": "Very dissatisfied"
                }
            ]
        },
        {
            "question": "Which of the following words would you use to describe our products? Select all that apply.",
            "question_type": "r10",
            "question_id": "q02",
            "question_select": "multi",
            "question_category": "product",
            "question_responses": [
                {
                    "value": "0",
                    "text": "Reliable"
                },
                {
                    "value": "1",
                    "text": "High Quality"
                },
                {
                    "value": "2",
                    "text": "Useful"
                },
                {
                    "value": "3",
                    "text": "Unique"
                },
                {
                    "value": "4",
                    "text": "Good Value for Money"
                },
                {
                    "value": "5",
                    "text": "Overpriced"
                },
                {
                    "value": "6",
                    "text": "Impractial"
                },
                {
                    "value": "7",
                    "text": "Ineffective"
                },
                {
                    "value": "8",
                    "text": "Poor Quality"
                },
                {
                    "value": "9",
                    "text": "Unreliable"
                }
            ]
        },
        {
            "question": "Did you find this survey useful?",
            "question_type": "r2",
            "question_id": "q03",
            "question_select": "single",
            "question_category": "product",
            "question_responses": [
                {
                    "value": "0",
                    "text": "Yes"
                },
                {
                    "value": "1",
                    "text": "No"
                }
            ]
        }
    ]

    return (
        <div className={Styles.container}>
            {survey.map((question, idx) =>{
                return <SurveyQuestion data={question} key={idx}/>
            })}
        </div>
    )
}
export default Survey;